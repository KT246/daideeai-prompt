...
function Field({ label, error, children }: { label: string; error?: string; children: ReactNode }) { return <label className="grid gap-1.5 text-sm font-medium"><span>{label}</span>{children}{error && <span className="motion-error text-xs text-red-500">{error}</span>}</label>; }
