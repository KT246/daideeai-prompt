...
export function SimplePromptBuilder({ mode, titleKey, descriptionKey }: { mode: TaskPromptMode; titleKey: "builder.title" | "debugging.title" | "system.title"; descriptionKey: "builder.description" | "debugging.description" | "system.description" }) {
  ...
  function generate() { if (!task.trim() || !context.trim()) return; const content = createTaskPrompt(task, context, mode, locale); setOutput(content); void fetch("/api/generated-prompts", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ generator: mode, content }) }); }
