...
async function config(): Promise<BootstrapConfig> {
  ...
  if (!username || !password || !url || !secretKey) return { error: "Missing environment variable: NEXT_PUBLIC_SUPABASE_URL, SUPABASE_SECRET_KEY, DEFAULT_ADMIN_USERNAME, or DEFAULT_ADMIN_PASSWORD." };
  if (!/^[a-zA-Z0-9._-]{3,40}$/.test(username)) return { error: "DEFAULT_ADMIN_USERNAME must use only letters, numbers, periods, underscores, or hyphens (3–40 characters)." };
  if (password.length < 8) return { error: "DEFAULT_ADMIN_PASSWORD must be at least 8 characters." };
...
async function bootstrap(): Promise<BootstrapResult> {
  ...
  if ("error" in values) return { ok: false, status: "skipped", message: values.error };
  ...
  async function syncAdmin(id: string): Promise<BootstrapResult> {
    ...
    if (passwordError) return { ok: false, status: "error", message: `Could not update the default account password: ${passwordError.message}` };
    const { error: profileError } = await admin.from("profiles").upsert({ id, username, display_name: username, role: "admin" });
    return profileError ? { ok: false, status: "error", message: `Could not update the admin role: ${profileError.message}` } : { ok: true, status: "updated", message: `Default account '${username}' already exists and was synchronized.` };
  }
  const { data: existing, error: lookupError } = await admin.from("profiles").select("id").ilike("username", username).maybeSingle();
  if (lookupError) return { ok: false, status: "error", message: `Could not check the default account: ${lookupError.message}` };
  ...
  if (createError || !created.user) {
    ...
    return { ok: false, status: "error", message: `Could not create the default account: ${createError?.message ?? "Supabase did not return a user."}` };
  }
  const { error: profileError } = await admin.from("profiles").upsert({ id: created.user.id, username, display_name: username, role: "admin" });
  return profileError ? { ok: false, status: "error", message: `Auth user was created but its profile could not be created: ${profileError.message}` } : { ok: true, status: "created", message: `Created default admin account '${username}'.` };
}
