...
const payloadSchema = z.object({ generator: z.enum(["crud", "builder", "debugging", "system-checking"]), content: z.string().min(50).max(30000) });
export async function POST(request: Request) {
  ...
  const { error } = await supabase.from("generated_prompts").insert({ user_id: user.id, generator: parsed.data.generator, content: parsed.data.content });
